...
【写作要求】
请写约380字的个性化报告，分4段，每段之间空一行：

第一段（定调）：
根据「${rating}」这个评级，用一两句话准确定调，要有画面感。如果是天才少年，要说出为什么罕见（结合具体数字）；如果是普通人，也要真诚地说出他的潜力在哪。不能用"综合评分xx分"这种干燥开头。

第二段（维度分析）：
重点剖析最强和最弱的维度，要结合具体分数说出为什么这个数据值得关注。比如"你的反应速度达到${scores.reaction}分，意味着你的视觉神经传导速度…"这样有实质内容的分析。要有逻辑，不能只是罗列数字。

第三段（游戏类型匹配）：
给出最适合的游戏类型推荐，且说清楚为什么他的数据组合最适合这类游戏。可以顺带说说他不那么适合什么类型，以及为什么（要有依据，不能臆造）。

第四段（角色方向 + 短板风险）：
根据他的数据组合，具体说明他最适合的游戏角色方向（如FPS的狙击手/入场手，MOBA的打野/ADC等），并点出最弱维度如果不加训练可能带来的具体影响（不要说废话，要有逻辑）。

第五段（训练建议 + 展望）：
针对最弱维度给出1-2条具体可操作的训练建议，要实用。最后根据评级结尾：
${rating.includes('天才') ? '- 强调他的天赋在同龄人中的稀缺性，表达VE和合作俱乐部对他的高度期待，语气让他感受到被认可的激动感' : rating.includes('潜力') ? '- 强调他距离职业门槛已经很近，鼓励他把天赋转化为实力，表达俱乐部持续关注' : '- 鼓励他坚持训练，说出通过努力缩小差距的具体方向，语气真诚不敷衍'}

输出纯文本，无标题，无markdown符号。语言自然流畅，像一个真正了解他数据的专家在跟他说话。约500字。`;

  const response = await axios.post(
    'https://api.minimax.chat/v1/text/chatcompletion_pro',
    {
      model: 'abab6.5s-chat',
      tokens_to_generate: 1600,
