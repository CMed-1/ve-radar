...
- 分位函数必须使用标准正态 CDF：`85` 分约 `95.4` 分位，避免旧版近似把 85 分推到 99% 附近
- **不是正式常模百分位**；页面文案必须写“内部参考分位”，并强调“基于内部匿名有效样本持续校准 / 仅作站内相对参考”
- 已开始通过 `test_results` 表累计匿名完成样本，后续应切到经验分位

### 综合加权分（2026-04）
- 综合分不再使用七维算术平均；统一使用 `public/js/score-model.js`
- 当前权重：Aim 32%、反应 20%、认知 14%、专注 11%、抑制 9%、视力 7%、色觉 7%
- 评级阈值仍是综合加权分：genius ≥90 且 4项≥90 且最低≥78；pro ≥76 且 3项≥80；normal ≥45
- 新测试入库和后台历史重算都必须使用同一模型，避免 `preview/report/admin/server/db` 口径分裂
...
- 后台“样本校准”Tab 只读展示 `test_results` 的站内分布：全体/设备分组，指标含综合分、七维分、RT、Aim KPM/命中率、GNG、动态视力、Grid/N-back、色觉等；它不自动改变前台报告
